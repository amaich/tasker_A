...
class TaskListView(ListView):
    ...
    queryset = TaskModel.objects.all()[:5]
...
class TaskUpdateView(View):
    def get(self, request, slug):
        ...
        form = TaskModel(instance=update_task)

    def post(self, request, slug):
        update_task = get_object_or_404(TaskModel, slug=slug)
        form = TaskModel(request.POST, instance=update_task)
        if form.is_valid():
            update_task = TaskModel(summary=form.cleaned_data['summary'],
                                 description=form.cleaned_data['description'],
                                 slug=form.cleaned_data['slug'],)
            new_task.save()
            return HttpResponseRedirect(reverse('task_app:task_detail', kwargs={'slug': request.POST.get('slug')}))
